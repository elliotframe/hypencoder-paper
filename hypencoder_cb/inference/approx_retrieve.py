...
class HypecoderGraphRetrieverHybrid(BaseRetriever):
    ...
    def __init__(
        ...
        query_max_length: int = 32,
        cache_file: Optional[str] = None,
        num_entry_points_random: int = 1000,
        num_entry_points_bm25: int = 1000,
        ncandidates: int = 50,
        max_iter: int = 16,
        early_stop: bool = True,
        dtype: Union[torch.dtype, str] = "float32",
        ...
        Args:
            ...
            cache_file (Optional[str], optional): If provided, the cache file
                ...
                the file does not exist, it will be created and the data will
                stored in it. Defaults to None.
            num_entry_points_random (int, optional): The number of randomly selected
                initial entry points.
                Defaults to 10_000.
            num_entry_points_bm25 (int, optional): The number of seeded
                initial entry points.
                Defaults to 10_000.
            ncandidates (int, optional): The number of candidates to explore
                as each step. Defaults to 50.
        ...
        self.batch_size = batch_size
        self.encoded_item_path = encoded_item_path
        self.num_entry_points_random = num_entry_points_random
        self.num_entry_points_bm25 = num_entry_points_bm25
        self.num_entry_points = self.num_entry_points_bm25 + self.num_entry_points_random
        self.ncandidates = ncandidates
        self.max_iter = max_iter
        self.query_max_length = query_max_length
        self.early_stop = early_stop
        ...
        self.retriever = self.index.bm25(k1=self.k1, b=self.b, num_results = self.num_entry_points_bm25)
        # if self._index_exists():
        #     self._load_index()
        # else:
        #     self.fit(items_from_ir_dataset(ir_dataset))
    ...
    def set_parameters(self, num_entry_points_bm25, num_entry_points_random, ncandidates, max_iter):
        self.num_entry_points_random = num_entry_points_random
        self.num_entry_points_bm25 = num_entry_points_bm25
        self.num_entry_points = self.num_entry_points_bm25 + self.num_entry_points_random
        self.ncandidates = ncandidates
    ...
    def _set_entry_points_hybrid(self, query):
        """
        Set entry points using both BM25 query-conditioned selection and random selection.
        First n entry points are selected based on BM25 relevance to the query,
        and another n entry points are selected randomly.
        
        Args:
            query: Search query for BM25-based entry point selection
        """
        # Get top n items from BM25
        top_item_ids = self.query(query)
        
        # Get indices for BM25-selected items
        bm25_indices = torch.tensor(
            [self.item_id_to_index[item_id] for item_id in top_item_ids],
            dtype=torch.long,
            device=self.device
        )
        
        # Get n random indices (excluding BM25-selected ones)
        random.seed(43)
        bm25_indices_set = set(bm25_indices.cpu().tolist())
        available_indices = [
            idx for idx in range(self.encoded_item_embeddings.shape[0])
            if idx not in bm25_indices_set
        ]
        random_indices = torch.tensor(
            random.sample(available_indices, self.num_entry_points_random),
            dtype=torch.long,
            device=self.device
        )
        
        # Combine both sets of indices
        self.entry_point_indices = torch.cat([bm25_indices, random_indices])
        
        # Get embeddings for all entry points
        self.entry_point_embeddings = self.encoded_item_embeddings[self.entry_point_indices]
        
        # Get IDs for all entry points
        random_ids = [self.ids[idx] for idx in random_indices]
        self.entry_point_ids = top_item_ids + random_ids
        
        print(f"Selected {len(top_item_ids)} query-conditioned entry points (BM25) + {len(random_ids)} random entry points")
...
class HypecoderGraphRetrieverBM25Twice(BaseRetriever):
    ...
    def __init__(
        ...
        Args:
            ...
            k1 (float, optional): Term frequency saturation parameter (default: 1.5)
            b: (float, optional): Length normalisation parameter (default: 0.75)
            alpha: (float, optional): Weight given to Hypencoder scores (default: 0.7)
        """
    ...
    def set_parameters(self, num_entry_points, ncandidates, max_iter, alpha):
        self.num_entry_points = num_entry_points
        self.ncandidates = ncandidates
        self.max_iter = max_iter
        self.retriever = self.index.bm25(k1=self.k1, b=self.b, num_results = self.num_entry_points)
